...
- All apps and packages are in `apps/` or `packages/`
- Use TypeScript strict mode; ES2022 target with NodeNext module resolution
- Docker Compose for local dev and production deployment
- PostgreSQL + Redis as required infrastructure
- Prisma for database ORM and migrations
- WhatsApp engine adapters live in `packages/engines/src/adapters/`
- Domain logic lives in `packages/core/` with ports defined as TypeScript interfaces

## Verification

- `turbo lint` for linting across all packages
- `turbo test` for tests
- `turbo typecheck` for type checking
- Scripts in `scripts/` for contract checks and API boundary validation

## Child DOX Index

| Path | Scope |
|------|-------|
| `apps/AGENTS.md` | Application layer: admin (Next.js), api (NestJS), worker (BullMQ) |
...
| `plugins/AGENTS.md` | Plugin system and example plugins |
| `tools/AGENTS.md` | Development tools |
| `landing/AGENTS.md` | Landing page assets |
