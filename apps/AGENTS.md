...
- Profile status responses expose only an allowlisted connection-alert code and timestamp. Terminal disconnects persist that alert atomically inside profile settings; successful connection clears it and intentional disconnect records a non-error manual pause. Do not expose raw provider errors, keys, or session data through this alert.
- Reconnect attempts share a profile-wide budget and reset only on an actual ready connection. Forbidden, replaced, mismatched, bad-session and auth-storage failures stop for operator review. Manual disconnect persists disconnected state, preserves credentials, and cancels queued retries and stale ready callbacks. Only explicit connect may resume deliberately disconnected profiles. Baileys auth is serialized into the existing profile `sessionData` column, with no schema migration.
...
- Provider acknowledgement writes require a bounded, nonempty message ID, an accepted status, and the active profile ID. Retry only recognized transient database conflicts; never log provider message IDs.
- Sender resolution first checks same-profile stored messages for a unique phone JID linked to the exact original provider LID. Conflicting historical mappings stay unresolved; capped history cannot establish uniqueness. Known historical matches do not query WhatsApp.
- Payment Monitor profile-message, media, sender-resolution, conversation-list/message-list and group-list routes verify the selected profile or conversation belongs to the authenticated organization before service execution. Media batches reject missing or cross-profile IDs as one request and never return partial evidence.
- Conversation read, archive, unarchive, mute, pin, clear-messages and delete mutations verify conversation ownership through profile, workspace and organization before service execution. Authorization remains organization-wide, so another profile in the same organization is allowed.
- Conversation detail verifies conversation ownership before validating its optional message limit. The limit defaults to 50 and accepts only canonical decimal integers from 1 through 100; successful responses retain chronological messages and the existing object shape.
- Conversation message reads, single-message detail and local message deletion verify organization ownership before service execution. Message ownership also requires the stored message profile to match its conversation profile; conversation cursors belong to the selected conversation. Conversation message limits remain optional, reach the service as integers, default to 50 when omitted and reject malformed integer values before service execution.
- Direct image, video, audio, document, location, contact and poll sends verify body.profileId organization ownership before DTO validation, persistence or engine lookup. Tenant resource selectors must be strings; structured body values fail before ownership lookup. Text, reply and reaction remain separate because their optional or required message references also need sending-profile containment.
- Text, reply and reaction verify body.profileId ownership, then bind local database message references to that exact profile and a conversation with the same profile. Providers receive the stored WhatsApp message ID; quoted text also requires the normalized destination to match the referenced conversation.
...
- Signed Apps Script payment hooks send routing metadata without media data URLs. Record bounded delivery receipts in the existing hooks data volume using opaque message IDs, HTTP status and allowlisted receiver outcomes; never persist slips, message text, signatures or secrets in the trace. Unreported delivery failures are retained without raw media and relayed as compact signed error events when the receiver is reachable. The original signed group event is durably queued before delivery and retried on transient transport/HTTP/queue failures, with serialized requests, bounded immediate attempts and persisted backoff. Preserve its original source timestamp and require an explicit queue/duplicate acknowledgment before removing it. Keep a stable signed delivery ID across re-signing/restarts. Record safe redirect-hop status/host/timing and the returned receipt ID; do not retain one-time response URLs or forward a signed body to a different origin. Signing/access failures remain blocked for operator review; never retry financial submissions or WhatsApp sends through this path.
